...
describe("mini program sports feature switch", () => {
  test("hides the sports tab before config loads and whenever the switch is disabled", () => {
    expect(tabBarSource).toContain("const INITIAL_STATE = createTabState(true, false)");
  ...
  test("derives the active tab from the current route when rebuilding the list", () => {
    ...
    expect(tabBarSource).toContain("this.setData(createTabState(ordersEnabled, sportsEnabled))");
  });
